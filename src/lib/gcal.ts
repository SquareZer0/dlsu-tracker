// Extracted from api/gcal/route.ts so both the client-facing endpoint and
// the companion's snapshot (lib/snapshot.ts) can share one implementation
// instead of the companion making an internal HTTP call to its own server.
...
export async function getGCalData(now: Date): Promise<GCalData> {
